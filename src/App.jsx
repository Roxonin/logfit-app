...
/*  ※このサンドボックスにはTailwindのJITが無いため、bg-[#xxx]や            */
/*    text-white/40 のような任意値/スラッシュ透過度クラスは効かない。      */
/*    レイアウト(flex/grid/padding/rounded等)のみTailwindの既定クラスを   */
/*    使い、色・不透明度・フォントサイズは全てstyleで指定する。            */
/* ------------------------------------------------------------------ */
...
/* ウェイト種目の分類表示(新: equipment / 旧: machine真偽値 どちらにも対応) */
function equipmentLabel(entry) {
  if (entry.equipment) {
    return { machine: "マシン", free: "フリーウェイト", cable: "ケーブル" }[entry.equipment] || entry.equipment;
  }
  return entry.machine ? "マシン" : "フリーウェイト";
}
...
function buildReportText(session) {
  ...
  if (weightEntries.length) {
    ...
    weightEntries.forEach((e) => {
      lines.push(`・${e.name}(${e.bodyPart}) ${equipmentLabel(e)} ${e.weight}kg × ${e.reps}回 × ${e.sets}set`);
    });
    lines.push("");
  }
...
function buildCSV(sessions) {
  ...
  [...sessions]
    ...
    .forEach((s) => {
      ...
      s.entries.forEach((e) => {
        if (e.type === "weight") {
          rows.push([dateStr, "ウェイト", e.name, e.bodyPart, equipmentLabel(e), e.weight, e.reps, e.sets, "", "", "", s.rating, s.note || ""]);
        } else {
...
/*  記録中に他のことをして戻ってきても、入力途中の内容が消えないようにする   */
/* ------------------------------------------------------------------ */

const LAST_USER_KEY = "logift-last-user";

function draftKey(user) {
  return `logift-draft-${user}`;
}
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-4">
            <button onClick={clear} style={{ color: C.dim3, fontSize: 12, background: "none", border: "none", textDecoration: "underline" }}>
              クリア
            </button>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          ...
          {allowDecimal ? (
            <button onClick={() => press(".")} className="py-4 rounded-xl font-semibold" style={{ backgroundColor: C.panelHover, color: C.text, fontSize: 24, border: "none" }}>
              ．
            </button>
          ) : (
            <div />
...
function PillToggle({ options, value, onChange, color }) {
  const fontSize = options.length >= 3 ? 12.5 : 14;
  return (
    <div className="flex gap-2">
      {options.map((o) => (
        <button
          ...
          style={
            value === o.value
              ? { backgroundColor: color, color: C.dark, fontSize, border: "none" }
              : { backgroundColor: C.panelHover, color: C.dim2, fontSize, border: "none" }
          }
        >
          {o.label}
        </button>
...
function EntryForm({ type, initial, mode = "add", customExercises, onRegisterExercise, onSave, onDelete, onCancel }) {
  ...
  const [equipment, setEquipment] = useState(
    initial?.equipment ?? (initial?.machine === false ? "free" : "machine")
  );
  ...
  const fieldMeta =
    type === "aerobic"
      ...
      : {
          weight: { label: "重さ", unit: "kg", value: weight, setValue: setWeight, decimal: true, decimalPlaces: 1 },
  ...
  const confirmField = (val) => {
    ...
    if (idx >= 0 && idx < order.length - 1) {
      setActiveField(order[idx + 1]); // 決定したら次の項目へ自動遷移
    } else {
      setActiveField(null);
    }
  };
  ...
  const submit = () => {
    ...
    if (type === "aerobic") {
      ...
      onSave({ type, name: trimmed, bodyPart, equipment, weight, reps, sets });
    }
  };
  ...
  return (
    <div className="rounded-2xl p-4 mb-4" style={{ backgroundColor: C.panel, border: `1px solid ${accent}33` }}>
      ...
      <p style={{ fontSize: 11, color: C.dim4, marginBottom: 6, fontWeight: 700 }}>{type === "aerobic" ? "マシン / マシン無し" : "分類"}</p>
      <div className="mb-3">
        {type === "aerobic" ? (
          <PillToggle
            color={accent}
      ...
      <div className="flex-1">
        ...
        <p style={{ color: C.dim3, fontSize: 12 }}>
          {entry.type === "aerobic"
            ...
            : `${equipmentLabel(entry)} ・ ${entry.weight}kg × ${entry.reps}回 × ${entry.sets}set`}
        </p>
      </div>
...
function SessionScreen({ user, customExercises, onRegisterExercise, onFinish, onExit }) {
  const draft = loadDraft(user);
  const [type, setType] = useState(draft?.type || "weight");
  const [entries, setEntries] = useState(draft?.entries || []);
  const [showForm, setShowForm] = useState((draft?.entries || []).length === 0);
  const [editingIndex, setEditingIndex] = useState(null);
  const [formKey, setFormKey] = useState(0);
  const [restoredNotice, setRestoredNotice] = useState((draft?.entries || []).length > 0);

  // entriesかtypeが変わるたびに下書きとして自動保存する
  useEffect(() => {
    if (entries.length > 0) {
      saveDraft(user, { type, entries });
    } else {
      clearDraft(user);
    }
  }, [user, type, entries]);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: C.bg }}>
      ...
      <div className="px-5 pt-5" style={{ paddingBottom: 130 }}>
        {restoredNotice && (
          <div className="rounded-xl p-3 mb-4 flex items-center justify-between gap-2" style={{ backgroundColor: "rgba(61,214,198,0.10)" }}>
            <p style={{ color: C.text, fontSize: 12, lineHeight: 1.5 }}>前回の続き({entries.length}種目)を復元しました</p>
            <button
              onClick={discardDraft}
              style={{ color: C.dim2, fontSize: 11, background: "none", border: "none", textDecoration: "underline", flexShrink: 0 }}
            >
              破棄して最初から
            </button>
          </div>
...
function HistoryScreen({ sessions, user, customExercises, onRegisterExercise, onBack, onUpdateSessionEntries, onUpdateSessionMeta }) {
  ...
  const [editing, setEditing] = useState(null); // { sessionId, index }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 前回選んだ名前が残っていれば、ログイン画面を飛ばして自動的に復元する
        let lastUser = null;
        try {
        ...
        if (lastUser && u.includes(lastUser)) {
          setUser(lastUser);
          const s = await db.getSessions(lastUser);
          setSessions(s);
          ...
          setFeed(
            raw
              .map((s2) => ({
                ...
                rating: s2.rating,
                note: s2.note,
                summary: `ウェイト${s2.entries.filter((e) => e.type === "weight").length}種目・有酸素${s2.entries.filter((e) => e.type === "aerobic").length}種目`,
              }))
              .reverse()
          );
          setScreen("menu");
  ...
  const loadFeed = useCallback(async () => {
    try {
      ...
      const withSummary = raw
        ...
        .reverse(); // 古い→新しい順にしてFeedScreen側のreverse()と合わせる
      setFeed(withSummary);
    } catch (e) {
      console.error(e);
    }
